...
import type {
  AnimationData,
  AssetsTrackerModelMapType,
  AssetsTrackerAudioType,
  AssetsTrackerCubeMapType,
  AssetsTrackerFontType,
  AssetsTrackerHdrCubeMapType,
  AssetsTrackerHdriTextureType,
  AssetsTrackerTextureMapType,
  AudioType,
  FontType,
  HdrCubeMapType,
  HdriTextureType,
  SourceCubeMapDataType,
  SourceRecordType,
  SourceTextureData,
  TextureCloneType,
  TrackedModelDataType,
} from "./allTypes.js";
import { clone as skeletonClone } from "three/examples/jsm/utils/SkeletonUtils.js";

import type {
  CloneRequestTypeForModel,
  CloneRequestTypeForTexture,
} from "./assetsEntryType.js";

export class AssetsTracker {
  private static instance: AssetsTracker;
  public audioListener?: AudioListener;

  public modelsMap: AssetsTrackerModelMapType = {};
  public texturesMap: AssetsTrackerTextureMapType = {};
  public cubeTextureMap: AssetsTrackerCubeMapType = {};
  public hdrCubeTextureMap: AssetsTrackerHdrCubeMapType = {};
  public hdriTextureMap: AssetsTrackerHdriTextureType = {};
  public fontMap: AssetsTrackerFontType = {};
  public audioMap: AssetsTrackerAudioType = {};

  private assetsKeyToNameMap: Map<number, string>;
  private assetsNameToKeyMap: Map<string, number>;
  constructor() {
    this.assetsKeyToNameMap = new Map();
    this.assetsNameToKeyMap = new Map();
  }
  public static getInstance(): AssetsTracker {
  ...
  getModel(
    modelIdOrName: number | string,
    isCloned = false
  ): SourceRecordType | TrackedModelDataType {
    let modelName: string;

    if (typeof modelIdOrName === "number") {
      const name = this.assetsKeyToNameMap.get(modelIdOrName);
      if (!name) throw new Error(`Model ID ${modelIdOrName} not found`);
      modelName = name;
    } else {
    ...
    const baseName = modelName.replace(
      /(DeepClone|ShallowClone|SkeletonClone)\d+$/,
      ""
    );

    const modelRecord = this.modelsMap[baseName];
    if (!modelRecord) throw new Error(`Base model '${baseName}' not found`);
  ...
  getTexture(textureIdOrName: number | string, isCloned = false) {
    let textureName: string;

    // ----- Case 1: ID → Name -----
    if (typeof textureIdOrName === "number") {
      const name = this.assetsKeyToNameMap.get(textureIdOrName);
      if (!name) throw new Error(`Texture ID ${textureIdOrName} not found`);
      textureName = name;
    }
    ...
    if (!record) throw new Error(`Texture '${baseName}' not found.`);
    ...
    if (!clone) throw new Error(`Cloned texture '${textureName}' not found.`);
  ...
  setSourceTexture(data: SourceTextureData) {
  ...
  setCloneTexture(data: TextureCloneType) {
    const record = this.texturesMap[data.name];
    if (!record) throw new Error(`Texture source '${data.name}' not found.`);
  ...
  setModelSource(data: SourceRecordType) {
    let mixer: AnimationMixer | null = null;
    let actions: Record<string, AnimationAction> | null = null;
  ...
  setCloneModel(
    modelData: TrackedModelDataType,
    cloneType: "DeepClone" | "ShallowClone" | "SkeletonClone"
  ) {
  ...
  setCubeMap(data: SourceCubeMapDataType) {
  ...
  setHdrCubeMap(data: HdrCubeMapType) {
  ...
  setHdriTexture(data: HdriTextureType) {
  ...
  setCameraForPositionalAudio(camera: Camera) {
  ...
  // -------------------------------------------------------
  private _disposeObject(obj: Object3D) {
  ...
  private _isTextureStillUsed(tex: Texture): boolean {
    for (const modelName in this.modelsMap) {
      const record = this.modelsMap[modelName];
      if (!record) continue;

      // Source
      if (this._modelUsesTexture(record.source.sourceModel, tex)) return true;

      // Deep clones
      for (const clone of Object.values(record.clones.DeepClone)) {
        if (this._modelUsesTexture(clone.modelRef, tex)) return true;
      }
  ...
  private _modelUsesTexture(obj: Object3D, tex: Texture): boolean {
    let found = false;
    obj.traverse((child: any) => {
      if (found) return;
  ...
  private _isInsideScene(obj: Object3D, scene: Scene): boolean {
  ...
  disposeSourceModel(modelName: string) {
    const record = this.modelsMap[modelName];
    if (!record) return;

    const disposeCloneGroup = (group: Record<string, TrackedModelDataType>) => {
  ...
  disposeClone(cloneName: string) {
    const baseName = cloneName.replace(
      /(DeepClone|ShallowClone|SkeletonClone)\d+$/,
      ""
    );
    ...
    let cloneType: "DeepClone" | "ShallowClone" | "SkeletonClone" | null = null;
    let clone: TrackedModelDataType | undefined;
    ...
    if (clone.mixerRef) {
      ...
      if (clone.actionsRef) {
        Object.values(clone.actionsRef).forEach((action) => {
          ...
          if (clip) clone.mixerRef!.uncacheClip(clip);
        });
      }
    }
  ...
  disposeSourceTexture(name: string) {
  ...
  // -------------------------------------------------------
  private _sceneUsesTexture(scene: Scene, tex: Texture): boolean {
    let found = false;
  ...
  //Dispose CubeMaps
  disposeCubeTexture(name: string) {
  ...
  disposeHdrCubeTexture(name: string) {
  ...
  disposeHdriTexture(name: string, scene: Scene) {
    ...
    // Remove from scene
    if (scene.environment === tex.hdri) scene.environment = null;
    if (scene.background === tex.original) scene.background = null;
  ...
  disposeAudio(audioName: string) {
    ...
    try {
      ...
      if ((audio as any).source) {
        try {
          (audio as any).source.disconnect?.();
        } catch (_) {}
        (audio as any).source = null;
      }

      // Disconnect positional audio nodes safely
      if ((audio as any).panner?.disconnect) {
        try {
          (audio as any).panner.disconnect();
        } catch (_) {}
      }

      // Disconnect gain node
      if ((audio as any).gain?.disconnect) {
        try {
          (audio as any).gain.disconnect();
        } catch (_) {}
      }
  ...
  // -------------------------------------------------------
  disposeTexturesByScene(scene: Scene) {
    for (const texName in this.texturesMap) {
      ...
      const clonesToDispose = new Set<string>();

      for (const cloneName in record.clones) {
        const cloneTexRecord = record.clones[cloneName];
        if (!cloneTexRecord) continue;
  ...
  disposeCloneTexture(cloneName: string) {
...
export function CloneTexture(
  ...
  const record =
    assetsTracker.texturesMap[assetsTracker["assetsKeyToNameMap"].get(id)!];
  if (!record) throw new Error(`Texture with ID ${id} not found.`);
...
export function DeepClone(modelData: SourceRecordType, count: number) {
  const assetsTracker = AssetsTracker.getInstance();
  ...
  for (let i = 0; i < count; i++) {
    ...
    const trackedData: TrackedModelDataType = {
...
export function ShallowClone(modelData: SourceRecordType, count: number) {
  const assetsTracker = AssetsTracker.getInstance();
  const gltf = modelData.sourceModel;
...
export function SkeletonClone(
  modelData: SourceRecordType,
  count: number,
  deepClone: boolean = false,
  cloneGeometry: boolean = false
) {
  const assetsTracker = AssetsTracker.getInstance();
  const gltf = modelData.sourceModel;
        ...
        if (node.isMesh) {
          ...
          if (deepClone) {
            node.material = Array.isArray(node.material)
              ? node.material.map((mat: Material) => mat.clone())
              : node.material.clone();
...
function loadCloneAnimations(
  sourceAnimations: AnimationClip[],
  clone: Object3D
): AnimationData | null {
  if (!sourceAnimations || sourceAnimations.length === 0) return null;
