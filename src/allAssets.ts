import type { CloneType } from "./assetsLoader.js";
import {
  type AssetsTrackerAudioType,
  type AssetsTrackerCubeMapType,
  type AssetsTrackerFontType,
  type AssetsTrackerHdrCubeMapType,
  type AssetsTrackerHdriTextureType,
  type AssetsTrackerModelMapType,
  type AssetsTrackerTextureMapType,
  type FontType,
  type HdrCubeMapType,
  type HdriTextureType,
  type SourceCubeMapDataType,
  type SourceRecordType,
  type SourceTextureData,
  type TextureCloneType,
  type TrackedModelDataType,
} from "./allTypes.js";
...
export default class AllAssets {
  public static instance: AllAssets;

  public modelsData: AssetsTrackerModelMapType;
  public audios: AssetsTrackerAudioType;
  public textures: AssetsTrackerTextureMapType;
  public cubeTextures: AssetsTrackerCubeMapType;
  public hdrCubeTextures: AssetsTrackerHdrCubeMapType;
  public hdriTextures: AssetsTrackerHdriTextureType;
  public fonts: AssetsTrackerFontType;
  ...
  fetchModel(
    name: string,
    // isCloned: true,
    cloneType: CloneRequestTypeForModel,
    count: number
  ...
  fetchModel(
    ...
    if (cloneMatch) {
      ...
      if (!record) throw new Error(`Base model '${baseName}' not found`);

      const clone = record.clones[type][name];
      if (!clone) throw new Error(`Clone '${name}' not found`);
    ...
    switch (cloneType.methods) {
      ...
      case "SkeletonClone":
        SkeletonClone(
          source,
          cloneType.count ? cloneType.count : 1,
          (cloneType.deepClone = false),
          (cloneType.cloneGeometry = false)
        );
        break;
    }
    ...
    if (count && count > 1) {
      const clones: TrackedModelDataType[] = [];
      const total = count;

      for (let i = 0; i < total; i++) {
        const idx = startIndex + i;
        const createdName = `${name}${capitalize(
          cloneType.methods
        )}Clone${idx}`;
        const clone = record!.clones[cloneType.methods][createdName];
        if (!clone) throw new Error(`Clone '${createdName}' missing`);
        clones.push(clone);
      }
  ...
  getAllModelsArray(): (SourceRecordType | TrackedModelDataType)[] {
    const tracker = this.#assetsTracker;
    const result: Array<SourceRecordType | TrackedModelDataType> = [];
  ...
  getModelsObject(): AssetsTrackerModelMapType {
    ...
    const clean: AssetsTrackerModelMapType = {};
  ...
  fetchTexture(
    nameOrId: string | number,
    isCloned: boolean
  ): SourceTextureData | TextureCloneType;

  fetchTexture(nameOrId: string | number, isCloned?: boolean) {
    const tracker = this.#assetsTracker;
  ...
  getSourceTextures(...names: string[]) {
    const tracker = this.#assetsTracker;
    const result: SourceTextureData[] = [];
  ...
  getAllTexturesArray() {
    const tracker = this.#assetsTracker;
    const result: (SourceTextureData | TextureCloneType)[] = [];
  ...
  disposeCubeMap(name: string) {
    this.#assetsTracker.disposeCubeTexture(name);
  }
  disposeHdrCubeMap(name: string) {
  ...
  disposeSourceModel(name: string) {
    this.#assetsTracker.disposeSourceModel(name);
  }
  disposeCloneModel(name: string) {
    this.#assetsTracker.disposeClone(name);
  }
  disposeSourceTexture(textureName: string) {
    this.#assetsTracker.disposeSourceTexture(textureName);
  }

  disposeCloneTexture(textureName: string) {
...
type CloneKey = "DeepClone" | "ShallowClone" | "SkeletonClone";

function normalizeCloneType(t: string): CloneKey {
  switch (t) {
    case "DeepClone":
      return "DeepClone";
    case "ShallowClone":
